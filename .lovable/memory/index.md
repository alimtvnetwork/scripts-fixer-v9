...
Current version: v0.36.0 with 43 scripts, 81-model GGUF catalog, models orchestrator, bootstrap auto-discovery.
4-filter chain in model picker: RAM -> Size -> Speed -> Capability.
Bootstrap (install.ps1/install.sh): parses scripts-fixer-vN, parallel-probes v(N+1)..v(N+30), redirects to latest, then fresh-clones.
